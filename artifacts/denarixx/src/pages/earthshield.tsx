...
const TYPE_META: Record<string, { label: string; icon: React.FC<{ className?: string }>; color: string; bg: string }> = {
  flood: { label: 'Flood', icon: ({ className }) => <Droplets className={cn('text-blue-400', className)} />, color: 'text-blue-400', bg: 'bg-blue-500/10 border-blue-500/30' },
  wildfire: { label: 'Wildfire', icon: ({ className }) => <Flame className={cn('text-red-500', className)} />, color: 'text-red-500', bg: 'bg-red-500/10 border-red-500/30' },
  storm: { label: 'Storm', icon: ({ className }) => <Wind className={cn('text-slate-300', className)} />, color: 'text-slate-300', bg: 'bg-slate-500/10 border-slate-500/30' },
  earthquake: { label: 'Quake', icon: ({ className }) => <Activity className={cn('text-orange-400', className)} />, color: 'text-orange-400', bg: 'bg-orange-500/10 border-orange-500/30' },
  infrastructure: { label: 'Grid', icon: ({ className }) => <Zap className={cn('text-primary', className)} />, color: 'text-primary', bg: 'bg-primary/10 border-primary/30' },
  drought: { label: 'Drought', icon: ({ className }) => <TrendingUp className={cn('text-amber-500', className)} />, color: 'text-amber-500', bg: 'bg-amber-500/10 border-amber-500/30' },
};
...
function SeverityBand({ severity }: { severity: string }) {
  const label = severity === 'critical' ? 'CRITICAL' : severity === 'warning' ? 'WARNING' : 'MONITORING';
        ...
        <div className="p-5 flex-1 flex flex-col">
          ...
          <div className="mt-auto pt-4 border-t border-border/50 grid grid-cols-2 gap-2 text-xs">
            <div className="bg-secondary/30 rounded-lg p-2 text-center">
              <p className="text-muted-foreground uppercase tracking-widest text-[9px] mb-0.5">Population</p>
              <p className="font-bold text-white font-mono">{alert.affectedPopulation.toLocaleString()}</p>
            </div>
            <div className="bg-secondary/30 rounded-lg p-2 text-center">
              <p className="text-muted-foreground uppercase tracking-widest text-[9px] mb-0.5">Issued</p>
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      <PageHeader
        ...
        description="Predictive planetary modeling and autonomous disaster mitigation across Africa."
      />

      {/* Threat Overview Cards */}
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
        {[
          { label: 'Flood Vectors', count: floodCount, icon: Droplets, color: 'text-blue-400', bar: 'bg-blue-500', max: 10 },
          { label: 'Thermal Events', count: fireCount, icon: Flame, color: 'text-red-500', bar: 'bg-red-500', max: 5 },
          { label: 'Atmos. Events', count: stormCount, icon: CloudLightning, color: 'text-slate-300', bar: 'bg-slate-400', max: 8 },
          { label: 'Grid Stress', count: infraCount, icon: Zap, color: 'text-primary', bar: 'bg-primary', max: 12 },
          { label: 'Pop. at Risk', count: (totalAffected / 1000).toFixed(0) + 'K', icon: Shield, color: 'text-amber-400', bar: 'bg-amber-400', max: null },
        ].map((stat, i) => (
      ...
      <div className="flex items-center gap-6 mb-6 px-1">
        <span className="text-xs text-muted-foreground uppercase tracking-widest font-bold">Legend:</span>
        {[
          { color: 'bg-destructive', label: 'Critical' },
          { color: 'bg-amber-500', label: 'Warning' },
          { color: 'bg-green-500', label: 'Monitoring' },
        ].map(l => (
          <div key={l.label} className="flex items-center gap-2 text-xs text-muted-foreground font-medium">
            <div className={cn('w-3 h-3 rounded-sm', l.color)} /> {l.label}
          </div>
        ))}
          ...
          <AnimatePresence>
            ...
            {monitoring.length > 0 && (
              <motion.div key="monitoring" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                <SeverityBand severity="info" />
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  {monitoring.map(a => <AlertCard key={a.id} alert={a} />)}
                </div>
              </motion.div>
            )}
            {filteredAlerts.length === 0 && (
              <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
                <Card className="p-12 text-center border-dashed border-2 border-border/40">
                  ...
                  <p className="text-muted-foreground font-medium">No active threats for this filter.</p>
                </Card>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <div className="space-y-6">
          ...
          <Card className="p-6 bg-secondary/20">
            <h3 className="text-sm font-display font-bold text-white uppercase tracking-widest border-b border-border/50 pb-4 mb-5 flex items-center gap-2">
              <Shield className="w-4 h-4 text-primary" /> Regional Readiness Index
            </h3>
          ...
          <Card className="p-6 border-primary/20 bg-gradient-to-b from-primary/5 to-transparent">
            <h3 className="text-sm font-display font-bold text-white uppercase tracking-widest border-b border-border/50 pb-4 mb-5 flex items-center gap-2">
              <Globe className="w-4 h-4 text-primary" /> Spatial Risk Distribution
            </h3>
          ...
          <Card className="p-6">
            <h3 className="text-sm font-display font-bold text-white uppercase tracking-widest border-b border-border/50 pb-4 mb-5 flex items-center gap-2">
              <AlertTriangle className="w-4 h-4 text-primary" /> Incident Timeline
            </h3>
